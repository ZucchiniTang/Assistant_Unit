...
def train(epoch):
	...
	for batch_idx, (inputs, targets, occludes) in enumerate(trainloader):
		...
		occludes = occludes.type(torch.cuda.FloatTensor)
		...
		loss = criterion1(outputs_class, targets)
		#loss2 = criterion2(outputs_occ, occludes)
...
def test(epoch):
	...
	with torch.no_grad():
		...
		print('test Loss:', '{:3f}'.format(total_test_loss),'   ','test Acc:', '{:3f}'.format(total_test_Acc),'   ', 'test_top2 Acc:', '{:3f}'.format(total_test_Acc2))
