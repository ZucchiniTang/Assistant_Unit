...
class OCC_CIFAR10(VisionDataset):
	...
	Args:
		...
		download (bool, optional): If true, downloads the dataset from the internet and
			...
			downloaded again.
	"""
	base_folder = 'occluded-cifar-10-each-class-average'
	...
	def __init__(self, root, train=True,
		...
		for file_name in downloaded_list:
			...
			with open(file_path, 'rb') as f:
				entry = pickle.load(f,encoding='latin-1')
				self.data.append(entry['data'])
				self.targets.extend(entry['labels'])
				self.occludes.extend(entry['occludes'])
			print('extend:',len(self.targets),len(self.occludes) )
	...
	def __getitem__(self, index):
		...
		img, target, occludes= self.data[index], self.targets[index], self.occludes[index]
